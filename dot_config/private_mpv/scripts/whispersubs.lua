...
--local WHISPER_CMD = "whisper-cli -m /usr/share/whisper/ggml-models/medium.bin --threads 6 --language ja -tr --no-prints"
local WHISPER_CMD = "whisper-cli -m /usr/share/whisper/ggml-models/base.bin --threads 6 --language ja -tr --no-prints"
...
local START_AT_ZERO = false		 -- start creating subs from 00:00:00 rather than the current time position (local files only)
